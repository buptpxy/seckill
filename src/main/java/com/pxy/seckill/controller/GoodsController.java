...
//将商品列表页面缓存到redis中
    ...
    /**将商品详情页面静态化，静态的部分缓存在浏览器中，动态的部分通过ajax获取
    ...
    @ResponseBody //忘了 @ResponseBody后出现 Error resolving template [goods/to_detail/2]错误
    public Result<GoodsDetailVo> detail(HttpServletResponse response, @CookieValue(value = "token", required = false) String token,
