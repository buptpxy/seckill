...
import com.pxy.seckill.access.AccessLimit;
...
//            localOverMap.put(goods.getId(),false);
    ...
    @ResponseBody
    public Result<String> getPath(Model model, HttpServletResponse response,
    ...
    @ResponseBody
    public Result<Integer> seckill(Model model,HttpServletResponse response,
                                   @CookieValue(value = "token",required = false) String token,
        ...
        if (seckillService.checkPath(user,goodsId,path)){
            int stock = goodsService.getSeckillGoodsStock(goodsId);
            //如果库存为0或负数就返回秒杀结束
            if (stock<=0){
            ...
            //否则在redis中减库存
            goodsService.reduceStockInRedis(user.getId(),goodsId);
    ...
    @ResponseBody
    public Result<Long> seckillResult(Model model,HttpServletResponse response,
                                      @CookieValue(value = "token", required = false) String token,
                                      @RequestParam("goodsId") long goodsId){
        if(token==null){
            return Result.error(CodeMsg.SESSION_ERROR);
...
//     * 感觉没啥用，还是得访问redis，并不会减轻redis的压力
//     * @throws Exception
//     */
...
//    @ResponseBody
//    public Result<OrderInfo> seckill0(Model model,
//                                     @CookieValue(value = "token", required = false) String token,
//                                     @RequestParam("goodsId")long goodsId ){
